...
void * generarNumeroRandom(void * threadid) {
    int numeroAleatorio = rand() % 254+ 1; //Genera un número aleatorio entre 1 y 254
    printf("Hilo %d generó el numero aleatorio: %d\n",(int)threadid,numeroAleatorio);
...
int main() {
    ...
    for (int i = 0; i < NUM_THREADS; i++) {
        ...
        if (pthread_create(&hilos[i], NULL, generarNumeroRandom,  (void *)i) != 0) {
    ...
    for (int i = 0; i < NUM_THREADS; i++) {
        if (pthread_join(hilos[i], (void *)&random) != 0){
            ...
            printf("Padre recibe numero %d generado por hilo: %d \n",  random,i);
    }
    return 0;
}
